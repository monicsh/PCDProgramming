...
public final class StudentAnalytics{
    ...
    public double averageAgeOfEnrolledStudentsParallelStream(
    	...
    	//OptionalDouble avg = students.parallelStream().filter(s -> s.checkIsCurrent()).mapToDouble(a -> a.getAge()).average();    	
    	//return avg.getAsDouble();
    	
    	return Stream.of(studentArray).parallel().filter(s -> s.checkIsCurrent()).mapToDouble(a -> a.getAge()).average().getAsDouble();
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
    	...
    	Map<String, Long> lookup = Stream.of(studentArray).parallel().filter(s -> !s.checkIsCurrent()).
    			collect(Collectors.groupingBy(Student::getFirstName, Collectors.counting()));
    	
    	
    	Entry<String, Long> maxEntry = lookup.entrySet().stream()
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
            final Student[] studentArray) {
    	long count = Stream.of(studentArray).parallel().
